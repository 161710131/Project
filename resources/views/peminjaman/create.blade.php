...
@push('styles')
<link rel="stylesheet" href="{{asset('dalam/vendors/bower_components/lightgallery/dist/css/lightgallery.min.css')}}">
<style type="text/css">
  .table thead tr th{
      ...
      <div class="card-body">
        <form action="{{route('peminjaman.store')}}" method="post" id="insert_form" multiple>
          ...
          <div class="table-responsive">
            <center>
              <table id="item_table" class="table">
                <tr id="last" align="center">
                  ...
                  <th><button type="button" name="add" class="btn btn-primary btn-xs add" onclick="addrow()"><i class="fa fa-plus-square"></i></button></th>
                </tr>
              </table>
            </center>
            <br>
            ...
            <div align="right">
              <button type="submit" name="submit" class="btn btn-success btn-xs" value="Pinjam"> Rental</button>
            </div>
          </div>
...
<script type="text/javascript">
  //multiple insert
    function addrow(){
      ...
      html +='<td><button type="button" class="btn btn-xs" style="background-color : darkviolet" onclick="remove('+ no +')"><i class="fa fa-minus-square"></i></button></td></tr>';
      $('#last').after(html);
      $('.sl2').select2();
      $('.date').flatpickr({
